...
def main():
    ...
    if parsed.event is not None:
        ...
        core_event_names = {
            ...
            0xfa6: "Bridge_Credits",
        }

        for event_id in sorted(core_event_names.keys()):
            try:
                addr = lookup_event(rom, event_id)
            except ValueError:
                # Since there are gaps, just ignore when we try to disassemble an invalid event
            ...
            do_disassemble_event(rom, event_id, core_event_names[event_id])
            print("")
