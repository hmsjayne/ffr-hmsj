...
locations = ''
temp_spirtes = ''
with open("./primary_code/labels/locations.txt","r") as f:
    locations = f.readlines()
with open("./primary_code/labels/sprites.txt","r") as f:
...
class Map_Events:
    ...
    def writeMap(self, file):
        ...
        for c in self.chests:
            file.write(hex(c['ptr']) + " = Chest ID " + str(c['chest_id']) + " at (" + str(c['x_coord']) + "," + str(c['y_coord']) +")\n")
        file.write("\nNPC Count: " + str(len(self.NPCs)) +"\n")
        for c in self.NPCs:
            file.write(hex(c['ptr']) + " = NPC " + str(c['Sprite']) + " with event ID 0x" + c['event'].hex() + " at (" + str(c['x_coord']) + "," + str(c['y_coord']) +")\n")
        file.write("\nTile Count: " + str(len(self.special_tiles)) +"\n")
        for c in self.special_tiles:
            file.write(hex(c['ptr']) + " = Tile with event ID 0x" + c['event'].hex() + " at (" + str(c['x_coord']) + "," + str(c['y_coord']) +")\n")
        file.write("\nSprite Count: " + str(len(self.interactive_sprites)) +"\n")
        for c in self.interactive_sprites:
            file.write(hex(c['ptr']) + " = Sprite 0x" + c['event'].hex() + " at (" + str(c['x_coord']) + "," + str(c['y_coord']) +") \n")
        file.write("\n")
