...
class EnemyName(object):
    def __init__(self, stream: InputStream = None):
        if stream is None:
            self.namePtr = 0

        else:
            self.namePtr = stream.get_u32()

    def write(self, stream: OutputStream):
        stream.put_u32(self.namePtr)


class EnemyGraphics(object):
    def __init__(self, stream: InputStream = None):
        if stream is None:
            self.tileData = 0
            self.palette = 0
            self.tileArrangement = 0

        else:
            self.tileData = stream.get_u32()
            self.palette = stream.get_u32()
            self.tileArrangement = stream.get_u32()

    def write(self, stream: OutputStream):
        stream.put_u32(self.tileData)
        stream.put_u32(self.palette)
        stream.put_u32(self.tileArrangement)


class EnemyScript(object):
    def __init__(self, stream: InputStream = None):
        if stream is None:
            self.spell_chance = 0
            self.ability_chance = 0
            self.spell_1 = 0
            self.spell_2 = 0
            self.spell_3 = 0
            self.spell_4 = 0
            ...
            self.spell_chance = stream.get_u8()
            self.ability_chance = stream.get_u8()
            self.spell_1 = stream.get_u8()
            self.spell_2 = stream.get_u8()
            self.spell_3 = stream.get_u8()
            self.spell_4 = stream.get_u8()
            self.spell_5 = stream.get_u8()
            self.spell_6 = stream.get_u8()
            self.spell_7 = stream.get_u8()
            self.spell_8 = stream.get_u8()
            self.spell_null = stream.get_u8()
            self.ability_1 = stream.get_u8()
            self.ability_2 = stream.get_u8()
            self.ability_3 = stream.get_u8()
            self.ability_4 = stream.get_u8()
            self.ability_null = stream.get_u8()

    def write(self, stream: OutputStream):
        stream.put_u8(self.spell_chance)
        stream.put_u8(self.ability_chance)
        stream.put_u8(self.spell_1)
        stream.put_u8(self.spell_2)
        stream.put_u8(self.spell_3)
        stream.put_u8(self.spell_4)
        stream.put_u8(self.spell_5)
        stream.put_u8(self.spell_6)
        stream.put_u8(self.spell_7)
        stream.put_u8(self.spell_8)
        stream.put_u8(self.spell_null)
        stream.put_u8(self.ability_1)
        stream.put_u8(self.ability_2)
        stream.put_u8(self.ability_3)
        stream.put_u8(self.ability_4)
        stream.put_u8(self.ability_null)
